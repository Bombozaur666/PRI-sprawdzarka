...
def question(request, id):
    this_id = id
    this_question = QuestionXML.objects.get(id = this_id)
    no_answer = ''
    teacher_answers = []
    answers = []
    check = False
    if AnswerXML.objects.filter(question_id = this_id).exists():
        answers = AnswerXML.objects.filter(question_id = this_id, has_teacher_answered = False)
        if AnswerXML.objects.filter(question_id = this_id, has_teacher_answered = True).exists():
            teacher_answers = AnswerXML.objects.filter(question_id = this_id, has_teacher_answered = True)
            check = True
    else:
...
def add_answer(request, id):
    if request.method == "POST":
        ...
        if form.is_valid():
            object = form.save(commit=False)
            task = QuestionXML.objects.get(id = id)
